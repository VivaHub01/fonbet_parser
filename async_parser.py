...
countries = ['australia', '92', 'algeria', 'england', 'argentina', 'aruba', 'bangladesh', 'bahrain', 'belgium', 'brazil', 'vietnam',
             'thegambia', 'germany', 'gibraltar', 'greece', 'djibouti', 'egypt', 'israel', 'india', 'indonesia', 'ireland', 'iceland',
               'spain', 'italy', 'cyprus',  'costarica', 'malaysia', 'mali', 'malta', 'morocco', 'mexico', 'myanmar', 'netherlands', 'uae',
                 'oman', 'panama', 'portugal', 'russia', 'romania', 'saudiarabia', 'northernireland', 'senegal', 'singapore', 'sierraleone',
                   'thailand', 'trinidadandtobago', 'turkey', 'wales', 'finland', 'france', 'switzerland', 'scotland', 'southafrica', 'japan']
...
async def parser_country_page(url, country, driver, results):
    driver.get(url)
...
async def country_page(driver, results):
    for country in countries:
        url = f'https://fon.bet/sports/football/{country}'
        print(f'Parsing {url}...')
        await parser_country_page(url, country, driver, results)  # Парсим каждую страну с использованием Selenium


async def check_coefficient(driver, results):
...
async def main():
    ...
    await country_page(driver, results)
    await check_coefficient(driver, results)  # Вызываем функцию для проверки коэффициентов
